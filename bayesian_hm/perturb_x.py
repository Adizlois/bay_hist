import numpy as np
def perturb_x(x,desired_samples=12000000,perturb_sd=0.20,int_index=4,int_sd=3):
    if x.shape[0]>=desired_samples:
        perturb=np.random.normal(scale=perturb_sd, size=(x.shape[0],x.shape[1])).astype(np.float32)
        perturb[:,int_index]=np.round(perturb[:,int_index]+np.random.normal(scale=int_sd,size=perturb.shape[1]),0)
        return x+perturb
    else:
        ...
        perturb[:,:,int_index]=np.round(perturb[:,:,int_index]+np.random.normal(scale=int_sd,size=(perturb.shape[0],perturb.shape[1])),0)
